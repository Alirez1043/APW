#include <vector>
#include <cstdlib>
#include <iostream>
#include <cmath>
#define PI 3.14

using namespace std;

class IllegalArgumentException {};

class Shape {
public:
    Shape(int init_x, int init_y) : x(init_x), y(init_y) {}
    int get_x() const { return x; }
    int get_y() const { return y; }
    void move(int dx, int dy);

    virtual void scale(int s) = 0;
    virtual void print() = 0;
    virtual double area() = 0;

protected:
    int x;
    int y;
};

void Shape::move(int dx, int dy)
{
    x += dx;
    y += dy;
}

class Rect : public Shape {
public:
    Rect(int init_x, int init_y, int w, int h);
    virtual void scale(int s);
    virtual void print();
    double area() override ;
private:
    int width;
    int height;
};

Rect::Rect(int init_x, int init_y, int w, int h)
        : Shape(init_x, init_y)
{
    if (w <= 0 || h <= 0)
        throw IllegalArgumentException();
    width = w;
    height = h;
}

void Rect::scale(int s)
{
    width *= s;
    height *= s;
}

void Rect::print()
{
    cout << "Rect: " << x << ',' << y << ',' << width << ',' << height << endl;
}

double Rect::area() {
    return width * height;
}

class Circle : public Shape {
public:
    Circle(int init_x, int init_y, int r);
    virtual void scale(int s);
    virtual void print();
    double area() override;
private:
    int radius;
};

Circle::Circle(int init_x, int init_y, int r)
        : Shape(init_x, init_y)
{
    if (r <= 0)
        throw IllegalArgumentException();
    radius = r;
}

void Circle::scale(int s)
{
    radius *= s;
}

void Circle::print()
{
    cout << "Circle: " << x << ',' << y << ',' << radius << endl;
}

double Circle::area() {
    return PI * radius * radius;
}

class Triangle: public Shape{
public:
    Triangle(int init_x, int init_y, int side);
    void scale(int s) override;
    void print() override;
    double area() override;
private:
    int m_side;
};


Triangle::Triangle(int init_x, int init_y, int side) : Shape(init_x, init_y){
    if (side < 0){
        throw IllegalArgumentException();
    }
    m_side = side;
}

void Triangle::scale(int s) {
    m_side *= s;
}

void Triangle::print() {
    cout << "Triangle: " << x << ',' << y << ',' << m_side << endl;
}

double Triangle::area() {
    return sqrt(3) / 4 * m_side * m_side * m_side;
}

class Line : public Shape{
public:
    Line(int init_x, int init_y, int length, int angle);
    void print() override;
    void scale(int s) override;
    double area() override;

private:
    int m_angle;
    int m_length;
};

Line::Line(int init_x, int init_y, int length, int angle): Shape(init_x, init_y){
    if (length < 0){
        throw IllegalArgumentException();
    }
    m_length = length;
    m_angle = angle;
}

void Line::print() {
    cout << "Line: " << x << ',' << y << ',' << m_length << ',' << m_angle << endl;
}

void Line::scale(int s) {
    m_length *= s;
}

double Line::area() {
    return 0;
}

int main()
{
    vector<Shape*> shapes;

    while (true) {
        cout << "r. New Rectangle\n";
        cout << "c. New Circle\n";
        cout << "t. New Triangle\n";
        cout << "l. New Line\n";
        cout << "p. Display Shapes\n";
        cout << "m. Move All\n";
        cout << "s. Scale All\n";
        cout << "a. Area\n";
        cout << "q. Exit\n";

        char ch;
        cin >> ch;

        if (ch == 'r') {
            int x, y, w, h;
            cout << "enter x, y, width, and height: ";
            cin >> x >> y >> w >> h;

            Rect *rect = new Rect(x, y, w, h);
            shapes.push_back(rect);

        } else if (ch == 'c') {
            int x, y, r;
            cout << "enter x, y, and radius: ";
            cin >> x >> y >> r;

            Circle *circ = new Circle(x, y, r);
            shapes.push_back(circ);

        } else if(ch == 't'){
            int x, y, side;
            cout << "enter x, y and side:";
            cin >> x >> y >> side;

            Triangle *triangle = new Triangle(x, y, side);
            shapes.push_back(triangle);
        }else if(ch == 'l') {
            int x, y, length, angle;
            cout << "enter x, y, length and angel:";
            cin >> x >> y >> length >> angle;

            Line *line = new Line(x, y, length, angle);
            shapes.push_back(line);
        }else if (ch == 'p') {
                for (int i = 0; i < shapes.size(); i++)
                    shapes[i]->print();
                cout<< endl;
        } else if (ch == 'm') {
            int dx, dy;
            cout << "enter dx, dy: ";
            cin >> dx >> dy;
            for (int i = 0; i < shapes.size(); i++)
                shapes[i]->move(dx, dy);

        } else if (ch == 's') {
            int s;
            cout << "enter s: ";
            cin >> s;
            for (int i = 0; i < shapes.size(); i++)
                shapes[i]->scale(s);
        }else if (ch == 'a'){
            for (int i = 0; i < shapes.size(); ++i) {
                cout << "#" << i+1 << " area is: " << shapes[i]->area() << endl;
            }
            cout << endl;
        }
        else if (ch == 'q') {
            for (int i = 0; i < shapes.size(); i++)
                delete shapes[i];
            break;
        }
    } // while (true)
}


